...
public class ConferenceApi {
	...
	public List<Conference> filterPlayground() {
		...
		query = query.filter("city =", "Tokyo").filter("seatsAvailable <", 10)
				.filter("seatsAvailable >", 0).order("seatsAvailable")
	...
	public Announcement getAnnouncement() {
		...
		MemcacheService memcacheServise = MemcacheServiceFactory
				.getMemcacheService();
		String annountcementKey = Constants.MEMCACHE_ANNOUNCEMENTS_KEY;
		Object massage = memcacheServise.get(annountcementKey);
		if (massage != null) {
			return new Announcement(massage.toString());
		}
		return null;
	}
